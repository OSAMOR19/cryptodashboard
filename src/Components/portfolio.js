...
const CryptoPortfolio = () => {
  ...
  const renderCategoryCard = (category) => (
    <Card
      className="mb-3 rounded-5 card-animate border-0"
  ...
  const renderCoinCard = (coin) => (
    <Card
      className="mb-3 border-0 rounded-4 card-animate"
      style={{ backgroundColor: "#F6F6F6" }}
    >
  ...
  const renderNFTCard = (nft) => (
    <Col xs={6} className="mb-3">
      <Card
        className="border-0 rounded-4 h-100 card-animate"
        style={{ backgroundColor: "#F6F6F6" }}
      >
    ...
    <Container fluid className="mt-3">
      <style>
        ...
        @import url('https://fonts.googleapis.com/css2?family=Geist&display=swap')
          ...
          .profit-pill {
            background-color: rgba(5, 254, 82, 0.07);
            color: #05FE52;
